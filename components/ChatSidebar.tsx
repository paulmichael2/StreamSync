...
export default function ChatSidebar({ roomId, username, participants }: ChatSidebarProps) {
  ...
  // Subscribe to chat messages via Pusher
  useEffect(() => {
  ...
  return (
    <aside className="flex flex-col h-full bg-[#0d0d0d] border-l border-white/[0.08]">
      {/* Tab switcher */}
      <div className="flex-shrink-0 px-4 py-3 border-b border-white/[0.08]">
        <div className="flex gap-1 p-1 bg-white/5 rounded-xl">
          <button
            onClick={() => setTab('chat')}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer ${tab === 'chat' ? 'bg-brand-red text-white' : 'text-white/50 hover:text-white'}`}
          >
            <MessageCircle size={13} /> Chat
          </button>
          <button
            onClick={() => setTab('users')}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer ${tab === 'users' ? 'bg-brand-red text-white' : 'text-white/50 hover:text-white'}`}
          >
            <Users size={13} /> Viewers
            {participants.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[10px] rounded-full bg-white/20">{participants.length}</span>
            )}
          </button>
        </div>
      </div>
                  ...
                  <div className={`flex gap-2 ${msg.username === username ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`flex flex-col max-w-[75%] ${msg.username === username ? 'items-end' : 'items-start'}`}>
                      <div className="flex items-baseline gap-1.5 mb-0.5">
                        <span className="text-[11px] font-semibold" style={{ color: getUserColor(msg.username) }}>
                          {msg.username === username ? 'You' : msg.username}
                        </span>
                        <span className="text-[10px] text-white/25">{formatTs(msg.timestamp)}</span>
                      </div>
                      <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed ${msg.username === username ? 'bg-brand-red text-white rounded-tr-sm' : 'bg-white/8 text-white/90 rounded-tl-sm'}`}>
                        {msg.message}
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3">
          {participants.length === 0 ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-white/30 px-1 mb-3">{participants.length} {participants.length === 1 ? 'person' : 'people'} watching</p>
              {participants.map((p) => (
                <div key={p.id} className="flex items-center gap-3 px-3 py-2.5 bg-white/5 rounded-xl">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: getUserColor(p.username) }}>
                    {p.username.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">
                      {p.username}
                      {p.username === username && <span className="ml-1.5 text-xs text-white/40 font-normal">(you)</span>}
                    </p>
                  </div>
